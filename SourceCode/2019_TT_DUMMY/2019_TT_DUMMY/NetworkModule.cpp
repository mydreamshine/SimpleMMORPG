...
void Worker_Thread()
{
	while (true) {
		...
		if (FALSE == ret) {
			...
			if (64 == err_no) DisconnectClient((int)ci);
...
void Adjust_Number_Of_Client()
{
	...
	if (high_resolution_clock::now() < last_connect_time + 500ms) return;
	...
	cs_packet_login_dummy packet;
	packet.size = sizeof(packet);
	packet.type = CS_LOGIN_DUMMY;
...
{
	for (int i = 0; i < num_connections; ++i) {
		point_cloud[i * 2] = (float)g_clients[i].x;
		point_cloud[i * 2 + 1] = (float)g_clients[i].y;
	}
